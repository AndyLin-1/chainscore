...
export default function HelloNear() {
  ...
  useEffect(() => {
    ...
    wallet.viewMethod({ contractId: CONTRACT, method: 'get_test', args: {id:"0"}})
      .then(greeting => {setGreeting(greeting.encryptedTest); console.log(greeting)});
  }, [wallet]);
  ...
  const storeGreeting = async () => {
    setShowSpinner(true);
    await wallet.callMethod({ contractId: CONTRACT, method: 'create_test', args: { id: "0", encryptedTest: newGreeting } });
    // const greeting = await wallet.callMethod({ contractId: CONTRACT, method: 'get_test', args: {id: '0'}});
